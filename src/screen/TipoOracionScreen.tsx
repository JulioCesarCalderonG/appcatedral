...
interface Props extends StackScreenProps<RootStackParams, 'TipoOracion'> { };

const {height,width} = Dimensions.get('window');

interface TipoOracion {
  id: number,
  titulo: string,
  descripcion:string,
  icono: any
...
const TipoOracionScreen = ({route,navigation}:Props) => {

  const listTipoOracion: TipoOracion[] = [
    {
      id: 1,
  ...
  return (
    <View style={{flex:1, backgroundColor:'#fff'}}>
        ...
        {
          listTipoOracion.map((resp, index) => {
...
export default TipoOracionScreen